			...
			<?php
				...
				while($row = mysql_fetch_array($result)){
					if($k == $row['id_menu']){
						...
						<p>Название элемента меню</p>  
     					<textarea name=\"elem_name\">".($row['title'])."</textarea>
  						<input type=\"hidden\" name=\"id_1\" value=\"".$row['id_menu']."\">
  						...
  						while($row_2 = mysql_fetch_array($result_2)){
							if($row['id_menu'] == $row_2['id_menu']){
								echo "<p>Заголовок элемента на странице</p>  <textarea name=\"title\">".($row_2['title'])."</textarea>";
     							if(($k > 1)&&($k < 5)){
     								echo "<p>Текст</p> <textarea  style=\"display:none;\" name=\"text\">".($row_2['content'])."</textarea><input type=\"hidden\" name=\"id_2\" value=\"".$row_2['id_main']."\">";
  								}
  								else{
  									echo "<p>Текст</p> 
					...
					echo "<input type=\"submit\" value=\"изменить\" name=\"edit\" onclick=\"this.form.action = 'edit_menu.php'\"; this.form.submit(); > 
      					<input type=\"submit\" value=\"удалить\" name=\"delete\" onclick=\"this.form.action = 'delete_menu.php'\"; this.form.submit(); > </form><br></div>";
