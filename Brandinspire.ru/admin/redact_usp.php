		...
		<div class="redaction">
			<?php
				...
				while($row = mysql_fetch_array($result)){
					if($k == $row['id_usp']){
						echo "<div class=\"elem\">";
						echo "<form action=\"edit_usp.php\" method=\"post\">
						<p>Заголовок</p>  
     					<textarea name=\"elem_name\">".($row['title'])."</textarea> 
     					<p>Текст</p> 
     					<textarea name=\"content\">".($row['text'])."</textarea>
  						<input type=\"hidden\" name=\"id_1\" value=\"".$row['id_usp']."\">
  						<input type=\"hidden\" name=\"k\" value=\"".$k."\">";
						echo "<input type=\"submit\" value=\"изменить\" name=\"edit\" onclick=\"this.form.action = 'edit_usp.php'\"; this.form.submit(); > 
